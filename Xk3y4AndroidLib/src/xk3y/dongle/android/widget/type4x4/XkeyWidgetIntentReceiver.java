...
public class XkeyWidgetIntentReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if(intent.getAction().equals("xk3y.dongle.android.action.4x4.PREV_GAME")){
			updateWidgetPrevGameListener(context);
		}else if (intent.getAction().equals("xk3y.dongle.android.action.4x4.NEXT_GAME")){
			updateWidgetNextGameListener(context);
		}else if (intent.getAction().equals("xk3y.dongle.android.action.4x4.PLAY_GAME")){
			updateWidgetPlayGameListener(context);
		}else if (intent.getAction().equals("xk3y.dongle.android.action.4x4.RELOAD_GAME")){
	...
	private void updateWidgetNextGameListener(Context context) {
		...
		remoteViews.setOnClickPendingIntent(R.id.nextButton, XkeyWidgetProvider.buildNextButtonPendingIntent(context));
		XkeyWidgetProvider.pushWidgetUpdate(context.getApplicationContext(), remoteViews);
	}
	...
	private void updateWidgetPlayGameListener(Context context) {
		...
		remoteViews.setOnClickPendingIntent(R.id.playButton, XkeyWidgetProvider.buildPlayButtonPendingIntent(context));
		XkeyWidgetProvider.pushWidgetUpdate(context.getApplicationContext(), remoteViews);
	}
